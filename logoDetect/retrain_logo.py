...
def main(image_dir, output_graph, output_labels, summaries_dir, how_many_training_steps, testing_percentage, validation_percentage, 
          ...
          random_crop, random_scale, random_brightness, model_dir, learning_rate, checkpoint_directory, delete_checkpoint, 
          output_graph_init, output_labels_init):
  ...
  (train_step, cross_entropy, bottleneck_input, ground_truth_input,
   final_tensor) = add_final_training_ops(len(image_lists.keys()),
                                          final_tensor_name,
                                          bottleneck_tensor, learning_rate)
  ...
  train_writer = tf.compat.v1.summary.FileWriter(summaries_dir + '/train',
                                       sess.graph)
  validation_writer = tf.compat.v1.summary.FileWriter(summaries_dir + '/validation')
  ...
  try:
    saver.restore(sess, f"checkpoint_dir/model.ckpt")
    print("Checkpoint Found!")
  except:
    print("No Checkpoint Found!")

  # print(FLAGS.checkpoint_dir)
  # ckpt = tf.train.load_checkpoint(FLAGS.checkpoint_dir)
  ...
  for i in range(how_many_training_steps):
    ...
    if (i % eval_step_interval) == 0 or is_last_step:
      ...
      saver.save(sess, f'checkpoint_dir/model.ckpt')
      # Write out the trained graph and labels with the weights stored as constants.
      output_graph_def = tf.compat.v1.graph_util.convert_variables_to_constants(
          sess, graph.as_graph_def(), [final_tensor_name])

      if cross_entropy_value >= 0.015:
        with gfile.GFile(output_graph_init, 'wb') as f:
          f.write(output_graph_def.SerializeToString())
        with gfile.GFile(output_labels_init, 'w') as f:
      ...
      for i, test_filename in enumerate(test_filenames):
        if predictions[i] != test_ground_truth[i].argmax():
          ...
          MISCLASSIFIED_TEST_IMAGES.append('%s  %s' % (test_filename,
                              list(image_lists.keys())[predictions[i]]))
  ...
  return ('Final test accuracy = %.1f%% (N=%d), Cross Entropy Value: %s, Misclassified Images: %s' % (test_accuracy * 100, len(test_bottlenecks),
                  cross_entropy_value, MISCLASSIFIED_TEST_IMAGES))